...
- Project: JoripNote template
- Service name: JoripNote
- Description: 로그인한 프로젝트 멤버가 계층형 문서를 만들고 블록 단위로 작성하며 함께 관리하는 협업 문서 서비스
- Main features: 최초 관리자 설치 마법사, 문서 너비(좁게·기본·전체 화면), 공개 회원가입 및 기본 역할 정책, 내부 워크스페이스 IP 허용목록, 문서 트리·목록의 휴지통 이동, 24종 블록 편집기, 인라인 서식, 충돌 방지 자동 저장, 제목·본문 전체 검색, 최근·즐겨찾기·휴지통, 멤버 초대 및 역할 관리, 문서 버전 기록·복원, 블록 연결 댓글·멘션, 활동·개인 알림, 문서 템플릿, 문서별 보기·편집 권한
- Notion import: Owner와 Admin은 Markdown 단일 파일 또는 Notion 전체 내보내기 ZIP을 가져올 수 있습니다. 대용량 ZIP은 브라우저에서 분할·병렬 전송하고 문서 계층, CSV 데이터 표, 첨부파일을 복원하며 같은 ZIP은 완료 항목부터 이어받습니다. 현재 최대 ZIP/압축 해제 크기는 5GB, 항목 10,000개, 문서 5,000개이며 개별 대용량 첨부파일은 8MB 조각으로 저장합니다. 구조와 변환 경계는 `docs/editor-migration-plan.md`와 `docs/editor-snapshot-schema.md`에 기록합니다.
