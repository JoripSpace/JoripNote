...
<head>
  ...
  <link rel="stylesheet" href="/app.css?v=20260910-joripnote-8">
</head>
<body>
  ...
  <script src="/app.js?v=20260910-joripnote-13" defer></script>
</body>
</html>`;
...
.db-card{gap:8px;padding:10px 10px 8px;border-color:#deddd9;border-radius:6px;box-shadow:0 1px 2px rgba(28,27,24,.06)}
.db-card-title-row{display:grid;grid-template-columns:16px minmax(0,1fr);align-items:start;gap:7px}.db-card-page-icon{display:grid;place-items:center;height:25px;color:#898984}.db-card-page-icon .ui-icon{width:14px;height:14px}.db-card-title{min-height:25px;padding:2px 0;line-height:1.5}
.db-card-meta:empty{display:none}.db-card-field{display:flex;align-items:center;gap:6px;min-height:20px;color:#74736e;font-size:10px}.db-card-field .ui-icon{width:13px;height:13px;flex:none}.db-card-value{min-width:0;overflow:hidden;color:#5e5d58;font-size:11px;text-overflow:ellipsis;white-space:nowrap}.db-card-value.is-tag{padding:2px 6px;border-radius:4px;background:#eeeeda}.db-card-value.is-person{display:inline-flex;align-items:center;gap:5px}.db-card-avatar{display:grid;place-items:center;width:18px;height:18px;border-radius:50%;background:#e7e4df;color:#5f5c57;font-size:9px;font-weight:750}.db-card-footer{height:0;overflow:visible;opacity:0;transition:opacity .12s}.db-card:hover .db-card-footer,.db-card:focus-within .db-card-footer{opacity:1}
...
function validateDatabasePayload(content) {
  ...
  if (String(data.title || '').length > 120 || data.columns.length < 1 || data.columns.length > 12 || data.rows.length > DATABASE_MAX_ROWS) throw new HttpError(400, '데이터베이스는 최대 ' + DATABASE_MAX_ROWS + '개 작업과 12개 속성을 지원합니다.');
