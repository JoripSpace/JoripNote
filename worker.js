...
<head>
  ...
  <link rel="stylesheet" href="/app.css?v=20260911-joripnote-10">
</head>
<body>
        ...
        <div class="settings-grid">
          <div class="settings-card import-card">
            ...
            <div class="import-actions">
              ...
              <button id="notion-api-import-button" type="button" hidden aria-hidden="true" tabindex="-1"></button>
  ...
  <script src="/app.js?v=20260911-joripnote-19" defer></script>
</body>
</html>`;
...
.tree-toggle{position:absolute;top:2px;right:57px;display:grid;place-items:center;width:27px;height:27px;padding:0;border:0;border-radius:6px;background:transparent;color:#85847e}
.tree-row.has-children .tree-title{padding-right:90px}
...
.tree-row:not(.has-children):hover .tree-title,.tree-row:not(.has-children):focus-within .tree-title{padding-right:61px}
.tree-row.has-children:hover .tree-title,.tree-row.has-children:focus-within .tree-title{padding-right:90px}
...
.database-block{min-width:0;max-width:100%;overflow:hidden}.database-block .db-toolbar{display:flex;flex-wrap:wrap;align-items:center;gap:8px;max-width:100%;overflow:visible}.database-block .db-viewbar{display:flex;max-width:100%;overflow-x:auto;overscroll-behavior-x:contain}.database-block .db-board{width:100%;min-width:0;max-width:100%;overflow-x:auto;overscroll-behavior-x:contain}.database-block .db-board-column{flex:0 0 clamp(220px,28vw,300px);min-width:0}.database-block .db-card{min-width:0;max-width:100%}.database-block .db-card-title{min-width:0;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.database-block .db-card-value{min-width:0;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.database-block .db-card-meta{min-width:0}.database-block .db-board-more{width:100%;padding:8px;border:1px dashed var(--line);border-radius:6px;background:#fff;color:var(--muted);font-size:11px}.db-calendar{min-width:0}.db-calendar-heading{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}.db-calendar-heading strong{font-size:15px}.db-calendar-controls{display:flex;gap:5px}.db-calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border-top:1px solid var(--line);border-left:1px solid var(--line)}.db-calendar-weekday{padding:7px 6px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);color:var(--muted);font-size:11px;text-align:center}.db-calendar-day{display:flex;flex-direction:column;gap:3px;min-height:92px;padding:5px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);background:#fff}.db-calendar-day.outside{background:#fafaf8;color:#aaa}.db-calendar-day.today{background:#f3f7fd;box-shadow:inset 0 0 0 2px #6f9be8}.db-calendar-date{align-self:flex-end;color:#777;font-size:11px}.db-calendar-event,.db-calendar-undated-item{display:block;min-width:0;overflow:hidden;padding:4px 5px;border:0;border-radius:4px;background:#eef3fb;color:#285b9b;font-size:11px;text-align:left;text-overflow:ellipsis;white-space:nowrap}.db-calendar-event:hover,.db-calendar-undated-item:hover{background:#dfeafb}.db-calendar-undated{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px;padding:10px;border:1px dashed var(--line);border-radius:7px}.db-calendar-undated strong{width:100%;font-size:11px;color:var(--muted)}.db-calendar-undated-item{background:#f7f7f5;color:var(--ink)}
@media(max-width:760px){.database-block{padding:8px}.database-block .db-toolbar>*{min-width:0;flex:1}.database-block .db-toolbar .db-search-wrap{flex-basis:100%}.db-calendar-day{min-height:72px;padding:3px}.db-calendar-event{font-size:10px;padding:3px}.database-block .db-board-column{flex-basis:78vw}}
`;
...
async function bootstrap(){const publicMatch=location.pathname.match(/^\/public\/([A-Za-z0-9_-]{8,80})$/);if(publicMatch){await openPublicDocument(publicMatch[1]);return}const agentAction=new URLSearchParams(location.search).get('agent_action');try{const setup=await api('/api/setup-status');state.publicSignup=!!setup.public_signup_enabled;if(!setup.installed){showSetup();return}if(location.pathname==='/setup')history.replaceState({},'','/');const data=await api('/api/me');enterApp(data);if(agentAction==='notion-import'&&canManage())setTimeout(importAllFromNotionApi,0)}catch(error){if(error.status===401)showAuth();else{showAuth();alertBox('auth-alert',error.message)}}}
...
function renderBlocks(blocks){$('block-editor').replaceChildren();const visibleBlocks=mergeDatabaseBlocksForDisplay(blocks);(visibleBlocks.length?visibleBlocks:[newBlock()]).forEach((block,index)=>$('block-editor').append(blockRow(block,index)));renumberBlocks()}
...
function normalizeDatabaseView(model, raw){const view=raw||model.view||{};const mode=String(view.type||view.mode||'table').toLowerCase();return{...view,type:['table','board','calendar','timeline','gallery','list'].includes(mode)?mode:'table',mode:mode,groupBy:view.groupBy||view.group_by||model.view.groupBy||'',datePropertyId:view.datePropertyId||view.date_property_id||model.view.datePropertyId||'',filter:view.filter||{column:'',operator:'contains',value:''}}}
...
function renderDatabaseCalendar(wrap,body,view){const model=wrap._databaseModel;const dateColumn=model.columns.find(column=>column.id===view.datePropertyId)||model.columns.find(column=>column.type==='date');const shell=document.createElement('div');shell.className='db-calendar';if(!dateColumn){shell.innerHTML='<div class="db-empty">날짜 속성이 없어 캘린더를 표시할 수 없습니다.</div>';body.append(shell);return}if(!wrap._calendarCursor)wrap._calendarCursor=new Date();const cursor=new Date(Date.UTC(wrap._calendarCursor.getUTCFullYear(),wrap._calendarCursor.getUTCMonth(),1));const year=cursor.getUTCFullYear(),month=cursor.getUTCMonth();const heading=document.createElement('header');heading.className='db-calendar-heading';const title=document.createElement('strong');title.textContent=year+'년 '+(month+1)+'월';const controls=document.createElement('div');controls.className='db-calendar-controls';const prev=document.createElement('button');prev.type='button';prev.className='db-control';prev.textContent='‹';prev.setAttribute('aria-label','이전 달');prev.onclick=()=>{wrap._calendarCursor=new Date(Date.UTC(year,month-1,1));renderDatabaseBody(wrap)};const today=document.createElement('button');today.type='button';today.className='db-control';today.textContent='오늘';today.onclick=()=>{wrap._calendarCursor=new Date();renderDatabaseBody(wrap)};const next=document.createElement('button');next.type='button';next.className='db-control';next.textContent='›';next.setAttribute('aria-label','다음 달');next.onclick=()=>{wrap._calendarCursor=new Date(Date.UTC(year,month+1,1));renderDatabaseBody(wrap)};controls.append(prev,today,next);heading.append(title,controls);shell.append(heading);const grid=document.createElement('div');grid.className='db-calendar-grid';for(const label of ['일','월','화','수','목','금','토']){const day=document.createElement('div');day.className='db-calendar-weekday';day.textContent=label;grid.append(day)}const firstDay=new Date(Date.UTC(year,month,1)).getUTCDay();const lastDay=new Date(Date.UTC(year,month+1,0)).getUTCDate();const start=new Date(Date.UTC(year,month,1-firstDay));const rows=databaseRows(model,wrap._databaseQuery);for(let index=0;index<42;index+=1){const date=new Date(Date.UTC(start.getUTCFullYear(),start.getUTCMonth(),start.getUTCDate()+index));const key=date.toISOString().slice(0,10);const cell=document.createElement('div');cell.className='db-calendar-day'+(date.getUTCMonth()===month?'':' outside')+(key===new Date().toISOString().slice(0,10)?' today':'');const dayNumber=document.createElement('span');dayNumber.className='db-calendar-date';dayNumber.textContent=String(date.getUTCDate());cell.append(dayNumber);for(const row of rows){const parts=databaseDateParts(row.cells[dateColumn.id]);if(!parts.length||key<parts[0]||key>(parts[1]||parts[0]))continue;const chip=document.createElement('button');chip.type='button';chip.className='db-calendar-event';const titleColumn=model.columns.find(column=>column.type==='text')||model.columns[0];chip.textContent=databaseDisplayValue(titleColumn,row.cells[titleColumn.id])||'제목 없음';chip.title=String(row.cells[titleColumn.id]||'');chip.onclick=()=>openDatabaseRowPage(row,titleColumn,chip);cell.append(chip)}grid.append(cell)}shell.append(grid);const undated=rows.filter(row=>!databaseDateParts(row.cells[dateColumn.id]).length);if(undated.length){const noDate=document.createElement('div');noDate.className='db-calendar-undated';noDate.innerHTML='<strong>날짜 없음</strong>';const titleColumn=model.columns.find(column=>column.type==='text')||model.columns[0];for(const row of undated.slice(0,100)){const link=document.createElement('button');link.type='button';link.className='db-calendar-undated-item';link.textContent=String(row.cells[titleColumn.id]||'제목 없음');link.onclick=()=>openDatabaseRowPage(row,titleColumn,link);noDate.append(link)}shell.append(noDate)}body.append(shell)}
...
const renderMediaPreviewBase=renderMediaPreview;renderMediaPreview=(preview,type,value)=>{renderMediaPreviewBase(preview,type,value);if(type==='embed'){const frame=preview.querySelector('iframe');if(frame){const src=frame.src;frame.referrerPolicy='strict-origin-when-cross-origin';frame.removeAttribute('src');frame.src=src}}};
...
function blockDataFromRow(row){const el=row.querySelector('.block-content');const type=el.dataset.type;let content=el.textContent;let checked=row.querySelector('input[type=checkbox]')?.checked||false;if(el.dataset.database==='true'){
...
$('link-form').onsubmit=event=>{event.preventDefault();const href=safeWebUrl($('link-url').value);if(!href){alertBox('link-alert','http 또는 https로 시작하는 올바른 웹 주소를 입력하세요.');$('link-url').focus();return}const target=state.linkTarget,range=state.linkRange;if(!applyLinkToSelection(target,range,href)){alertBox('link-alert','선택한 텍스트에 링크를 적용하지 못했습니다. 다시 선택해 주세요.');return}target.dataset.rich='true';scheduleSave();closeLinkDialog('applied');$('inline-toolbar').hidden=true;target.focus()};
...
async function route(request, env) {
  ...
  if (path.startsWith('/api/')) {
    ...
    if (path === '/api/import/notion-api/audit' && request.method === 'GET') return auditNotionApi(request, env, actor);
  ...
  for (const sourceId of sources.keys()) {
    ...
    if (database) {
      ...
      for (const block of blocks.results || []) {
        try { const model = JSON.parse(block.content); importedRows += Array.isArray(model.rows) ? model.rows.length : 0; localVersion = Math.max(Number(localVersion || 0), Number(model.version || 0)); localColumns = model.columns || localColumns; localViews = model.views || localViews; } catch {}
      }
    }
    ...
    rows.push({ data_source_id: sourceId, title: notionRichText(source.title) || database?.title || 'Notion 데이터베이스', remote_rows: remoteRows, imported_rows: importedRows, property_count: Object.keys(source.properties || {}).length, imported_property_count: localColumns.length, views: views.map(view => ({ id: view.id, name: view.name, type: view.type, order: view.order })), imported_views: localViews.map(view => ({ id: view.id, name: view.name, type: view.type, order: view.order })), local_version: localVersion, findings });
  }
...
async function importNotionApiDataSource(request, env, actor, rawSourceId) {
  ...
  const allRows = pages.map(page => ({ id: 'row_' + normalizedNotionId(page.id), source_id: normalizedNotionId(page.id), cells: Object.fromEntries(columns.map(column => { const property = Object.values(page.properties || {}).find(item => item?.id === column.sourceId); const value = notionPropertyText(property); return [column.id, value.slice(0, 500)]; })) }));
