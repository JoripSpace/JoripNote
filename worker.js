...
<body>
  ...
  <script src="/app.js?v=20260910-joripnote-12" defer></script>
</body>
</html>`;
...
function clientCsvDatabase(text,title){const table=parseClientCsv(text);if(!table.length)return null;const width=Math.max(1,Math.min(12,...table.map(row=>row.length)));const headers=Array.from({length:width},(_,index)=>String(table[0][index]||('속성 '+(index+1))).slice(0,80));const columns=headers.map((name,index)=>{const values=table.slice(1,201).map(row=>String(row[index]??'').trim()).filter(Boolean);const isStatus=/상태|status/i.test(name);const isDate=/날짜|일시|date|time/i.test(name);const isNumber=/금액|수량|번호|number|count|price/i.test(name)&&values.length>0&&values.every(value=>!Number.isNaN(Number(value.replace(/[, ]/g,''))));const isUrl=values.length>0&&values.every(value=>/^https?:\/\//i.test(value));return{id:'col_'+index,name,type:isStatus?'select':isDate?'date':isNumber?'number':isUrl?'url':'text',options:isStatus?[...new Set(values)].slice(0,30):[]}});const rows=table.slice(1,10001).map((values,rowIndex)=>({id:'row_'+rowIndex,cells:Object.fromEntries(columns.map((column,columnIndex)=>[column.id,column.type==='number'?String(values[columnIndex]??'').replace(/[, ]/g,''):String(values[columnIndex]??'').slice(0,500)]))}));return{version:2,title:title.slice(0,120),columns,rows,views:[{id:'view_table',name:'전체 보기',mode:'table',groupBy:'',sortBy:'',sortDir:'asc',filter:{column:'',operator:'contains',value:''}},{id:'view_board',name:'상태별',mode:'board',groupBy:columns.find(column=>column.type==='select')?.id||columns[0].id,sortBy:'',sortDir:'asc',filter:{column:'',operator:'contains',value:''}}],view:{mode:'table',groupBy:'',sortBy:'',sortDir:'asc',filter:{column:'',operator:'contains',value:''}}}}
