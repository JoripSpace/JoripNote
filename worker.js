...
<head>
  ...
  <link rel="stylesheet" href="/app.css?v=20260903-joripnote-8">
</head>
<body>
  ...
  <dialog id="publish-dialog" aria-labelledby="publish-dialog-title">
    <section class="dialog-card publish-card">
      ...
      <div class="publication-options" role="radiogroup" aria-label="문서 공개 상태">
        <button id="publication-private-option" class="publication-option active" type="button" role="radio" aria-checked="true"><span class="publication-option-icon"><svg class="ui-icon" aria-hidden="true"><use href="#icon-lock"/></svg></span><span><strong>비공개</strong><small>워크스페이스 멤버만 볼 수 있어요.</small></span></button>
        <button id="publication-public-option" class="publication-option" type="button" role="radio" aria-checked="false"><span class="publication-option-icon"><svg class="ui-icon" aria-hidden="true"><use href="#icon-link"/></svg></span><span><strong>웹에 공개</strong><small>링크가 있는 누구나 로그인 없이 읽어요.</small></span></button>
      </div>
      <div class="publication-row"><div><strong id="publication-title">현재 비공개</strong><small id="publication-description">공개할 상태를 위에서 선택하세요.</small></div><button id="publication-toggle" class="button primary compact" type="button">웹에 공개하기</button></div>
  ...
  <script src="/app.js?v=20260903-joripnote-8" defer></script>
</body>
</html>`;
...
.publication-options{display:grid;grid-template-columns:1fr 1fr;gap:10px}.publication-option{display:grid;grid-template-columns:32px minmax(0,1fr);align-items:start;gap:10px;min-height:96px;padding:14px;border:1px solid var(--line);border-radius:10px;background:#fff;text-align:left}.publication-option:hover{background:#fafaf8}.publication-option.active{border-color:#59647f;background:#f5f6f8;box-shadow:0 0 0 2px rgba(89,100,127,.1)}.publication-option-icon{display:grid;place-items:center;width:30px;height:30px;border-radius:8px;background:#eeefF2;color:#59647f}.publication-option strong,.publication-option small{display:block}.publication-option strong{font-size:13px}.publication-option small{margin-top:5px;color:var(--muted);font-size:10px;line-height:1.45}.publication-row{background:#fafaf8}.publication-row #publication-toggle{flex:none}.publication-row #publication-description{line-height:1.45}@media(max-width:520px){.publication-options{grid-template-columns:1fr}.publication-option{min-height:76px}.publication-row{align-items:stretch;flex-direction:column}.publication-row #publication-toggle{width:100%}}
...
.demo-banner{position:sticky;z-index:19;top:0;display:flex;align-items:center;justify-content:center;gap:10px;min-height:42px;padding:8px 18px;border-bottom:1px solid #cbd9ff;background:#edf3ff;color:#3d4f78;font-size:12px;line-height:1.45;text-align:center}.demo-banner strong{flex:none;padding:3px 7px;border-radius:5px;background:#4869b1;color:#fff;font-size:10px;letter-spacing:.08em}.demo-banner+ .mobile-header{top:42px}.demo-banner~.editor-view .editor-toolbar{top:42px}@media(max-width:760px){.demo-banner{position:relative;min-height:0;align-items:flex-start;justify-content:flex-start;padding:10px 12px;text-align:left}.demo-banner span{font-size:11px}.demo-banner+ .mobile-header{top:0}.demo-banner~.editor-view .editor-toolbar{top:50px}}
`;
...
async function bootstrap(){const publicMatch=location.pathname.match(/^\/public\/([A-Za-z0-9_-]{8,80})$/);if(publicMatch){await openPublicDocument(publicMatch[1]);return}try{const setup=await api('/api/setup-status');state.publicSignup=!!setup.public_signup_enabled;state.demoMode=!!setup.demo_mode;if(!setup.installed){showSetup();return}if(location.pathname==='/setup')history.replaceState({},'','/');const data=await api('/api/me');enterApp(data)}catch(error){if(error.status===401)showAuth();else{showAuth();alertBox('auth-alert',error.message)}}}
function enterApp(data){state.user=data.user;state.role=data.membership.role;$('boot-view').hidden=true;$('setup-view').hidden=true;$('auth-view').hidden=true;$('public-view').hidden=true;$('app-view').hidden=false;$('demo-banner').hidden=!state.demoMode;$('profile-name').textContent=data.user.username;$('profile-avatar').textContent=data.user.username.slice(0,1).toUpperCase();const roleText=roleLabel[state.role]+(state.demoMode?' · 데모':'');$('profile-role').textContent=roleText;$('sidebar-role').textContent=roleText;$('new-root-document').hidden=!canEdit();$('list-new-document').hidden=!canEdit();$('duplicate-button').hidden=!canEdit();$('notion-import-button').hidden=!canEdit();$('publish-button').hidden=!canManage();$('open-invite').hidden=!canManage();$('members-permission').hidden=canManage();$('new-template-button').hidden=!canEdit();let collapsed=false;let documentWidth='default';try{collapsed=localStorage.getItem('qwerty_sidebar_collapsed')==='1';documentWidth=localStorage.getItem('joripnote_document_width')||'default'}catch{}setSidebarCollapsed(collapsed,false);setDocumentWidth(documentWidth,false);setInviteRoleOptions();loadTree();refreshNotificationBadge();routeFromLocation()}
...
function renderPublication(){const published=!!state.publication?.published;const url=state.publication?.public_url||location.origin+'/public/'+state.current.id;const privateOption=$('publication-private-option');const publicOption=$('publication-public-option');privateOption.classList.toggle('active',!published);publicOption.classList.toggle('active',published);privateOption.setAttribute('aria-checked',String(!published));publicOption.setAttribute('aria-checked',String(published));$('publication-title').textContent=published?'현재 웹에 공개됨':'현재 비공개';$('publication-description').textContent=published?'링크를 가진 누구나 이 문서를 읽을 수 있습니다.':'워크스페이스 멤버만 이 문서를 볼 수 있습니다.';$('publication-toggle').textContent=published?'비공개로 전환':'웹에 공개하기';$('publication-toggle').className='button '+(published?'subtle danger-text':'primary')+' compact';const link=$('publication-link');link.hidden=!published;link.querySelector('input').value=url;link.querySelector('button').onclick=async()=>{await navigator.clipboard.writeText(url);toast('공개 링크를 복사했습니다.')}}
$('publish-button').onclick=openPublicationDialog;$('close-publish').onclick=()=>$('publish-dialog').close();
async function setPublication(nextPublished){if(!state.current||nextPublished===!!state.publication?.published)return;if(!confirm(nextPublished?'이 문서를 웹에 공개할까요? 링크를 가진 누구나 로그인 없이 읽을 수 있습니다.':'이 문서를 비공개로 전환할까요? 기존 공개 링크에서는 더 이상 볼 수 없습니다.'))return;const controls=[$('publication-toggle'),$('publication-private-option'),$('publication-public-option')];controls.forEach(button=>button.disabled=true);try{state.publication=await api('/api/documents/'+state.current.id+'/publication',{method:nextPublished?'PUT':'DELETE'});renderPublication();toast(nextPublished?'문서를 웹에 공개했습니다.':'문서를 비공개로 전환했습니다.')}catch(error){alertBox('publish-alert',error.message)}finally{controls.forEach(button=>button.disabled=false)}}
$('publication-private-option').onclick=()=>setPublication(false);$('publication-public-option').onclick=()=>setPublication(true);$('publication-toggle').onclick=()=>setPublication(!state.publication?.published);
...
function validateDatabasePayload(content) {
  ...
  const rows = data.rows.map((row) => {
    ...
    for (const column of columns) {
      ...
      if (column.type === 'date' && normalized && !/^\d{4}-\d{2}-\d{2}$/.test(normalized)) throw new HttpError(400, '날짜 속성은 YYYY-MM-DD 형식이어야 합니다.');
      if (column.type === 'date' && normalized) {
        const date = new Date(normalized + 'T00:00:00Z');
        if (Number.isNaN(date.getTime()) || date.toISOString().slice(0, 10) !== normalized) throw new HttpError(400, '날짜 속성은 유효한 날짜여야 합니다.');
      }
