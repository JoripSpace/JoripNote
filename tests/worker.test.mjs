...
test('app shell, editor capabilities and security headers are served', async () => {
  ...
  assert.match(html, /id="demo-banner" class="demo-banner"/);
...
test('Notion member sync preserves encrypted emails and enriches people properties', async () => {
  ...
  globalThis.fetch = async input => {
    const url = String(input);
    if (url.includes('/v1/users?')) return Response.json({ results: [{ object: 'user', id: personId, type: 'person', name: '테스트 사용자', avatar_url: 'https://example.com/avatar.png', person: { email: 'user@example.com' } }], has_more: false, next_cursor: null });
  ...
  try {
    ...
    assert.equal(stored.name, '테스트 사용자');
    assert.notEqual(stored.email_ciphertext, 'user@example.com');
    const members = await call(env, '/api/notion-members', { headers: { cookie } });
    assert.equal(members.body.members[0].email, 'user@example.com');
    const imported = await call(env, '/api/import/notion-api/data-sources/' + sourceId, { method: 'POST', headers: auth(cookie), body: {} });
    assert.equal(imported.response.status, 200, JSON.stringify(imported.body));
    const model = JSON.parse(env.DB.database.prepare("SELECT content FROM document_blocks WHERE block_type='database'").get().content);
    assert.equal(model.rows[0].cells.col_1, '테스트 사용자 <user@example.com>');
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test('JoripNote demo auto-login seeds the preview and cron reset is isolated and idempotent', async () => {
  const env = envWithDb();
  ...
  const wrongHost = await call(env, '/__joripnote_demo/reset', { method: 'POST' });
  assert.equal(wrongHost.response.status, 404);

  const reset = await call(env, '/__joripnote_demo/reset', { origin: CRON_ORIGIN, method: 'POST' });
  ...
  const repeated = await call(env, '/__joripnote_demo/reset', { origin: CRON_ORIGIN, method: 'POST', headers: { cookie: resetCookie } });
